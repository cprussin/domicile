//! Behaviour tests for `wc-config`, written before the implementation.
//!
...
use wc_config::{Config, ConfigError, ConfigStore, ShellRef};
...
fn parses_a_full_config() {
    ...
    assert_eq!(cfg.shell.package, ShellRef::Path(PathBuf::from("./shells/simple")));
    ...
    assert_eq!(
        cfg.shell.settings.get("show_clock").and_then(|v| v.as_bool()),
        Some(true)
    );
}
...
fn shellref_parses_bare_name() {
    assert_eq!("simple".parse::<ShellRef>().unwrap(), ShellRef::Name("simple".into()));
}
...
fn shellref_parses_paths() {
    for s in ["./x", "../x", "/opt/x", "a/b"] {
        ...
        assert!(matches!(parsed, ShellRef::Path(_)), "{s} should parse as a Path, got {parsed:?}");
    }
}
...
fn shellref_resolves_name_under_shells_dir() {
    ...
    assert_eq!(name.resolve(Path::new("/etc/loom/shells")), PathBuf::from("/etc/loom/shells/simple"));
}

#[test]
fn shellref_resolves_absolute_path_unchanged() {
    let p = ShellRef::Path(PathBuf::from("/opt/mychrome"));
    assert_eq!(p.resolve(Path::new("/etc/loom/shells")), PathBuf::from("/opt/mychrome"));
}
...
fn store_recovers_after_fixing_a_bad_edit() {
    ...
    assert!(store.last_error().is_none(), "error must clear once config is valid again");
}
...
fn missing_file_is_an_io_error() {
    let err = Config::load("/no/such/loom.toml").unwrap_err();
