...
pub enum ShellRef {
    ...
    Name(String),
    /// A filesystem path to a chrome package, e.g. `"./shells/simple"`.
    Path(PathBuf),
}
...
pub struct ShellConfig {
    ...
    /// Opaque settings handed to the chrome package verbatim. Loom does not
...
impl Default for CompositorConfig {
    fn default() -> Self {
        CompositorConfig { nested_size: (1280, 800) }
    }
}
...
impl ConfigStore {
    pub fn new(initial: Config) -> Self {
        ConfigStore { current: initial, last_error: None }
    }
...
pub fn watch(path: impl AsRef<Path>) -> Result<ConfigWatcher, ConfigError> {
    ...
    .map_err(|e| ConfigError::Io { path: path.display().to_string(), message: e.to_string() })?;

    watcher
        .watch(&dir, notify::RecursiveMode::NonRecursive)
        .map_err(|e| ConfigError::Io { path: dir.display().to_string(), message: e.to_string() })?;
