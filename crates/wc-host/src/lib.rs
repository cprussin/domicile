//! The Loom host orchestrator — the compositor's brain.
//!
...
impl Host {
    ...
    pub fn app_appeared(&mut self, title: Option<String>, size: (f64, f64)) -> (AppId, HostMessage) {
        self.next_id += 1;
        let app_id = format!("app-{}", self.next_id);
        self.apps.insert(app_id.clone(), App { app_id: app_id.clone(), title: title.clone(), size });
        let message = HostMessage::AppAppeared { app_id: app_id.clone(), title, size: [size.0, size.1] };
        (app_id, message)
    }
    ...
    pub fn app_resized(&mut self, app_id: &str, size: (f64, f64)) -> Option<HostMessage> {
        ...
        app.size = size;
        Some(HostMessage::AppResized { app_id: app_id.to_string(), size: [size.0, size.1] })
    }
    ...
    pub fn app_closed(&mut self, app_id: &str) -> Option<HostMessage> {
        ...
        Some(HostMessage::AppClosed { app_id: app_id.to_string() })
    }
    ...
    pub fn handle_chrome_message(&mut self, message: ChromeMessage) -> Result<(), HostError> {
        match message {
            ...
            ChromeMessage::PlacePortal { app_id, transform, size, z_index, visible } => {
    ...
    pub fn route_pointer(&self, x: f64, y: f64) -> InputDelivery {
        match self.scene.route_pointer(wc_scene::Point::new(x, y)) {
            PointerTarget::App { app_id, local } => {
                InputDelivery::App { app_id, local: (local.x, local.y) }
            }
            PointerTarget::Chrome { screen } => InputDelivery::Chrome { screen: (screen.x, screen.y) },
        }
    }
