...
fn shellref_resolves_name_under_shells_dir() {
    ...
    assert_eq!(name.resolve(Path::new("/etc/domicile/shells")), PathBuf::from("/etc/domicile/shells/simple"));
}

#[test]
fn shellref_resolves_absolute_path_unchanged() {
    let p = ShellRef::Path(PathBuf::from("/opt/mychrome"));
    assert_eq!(p.resolve(Path::new("/etc/domicile/shells")), PathBuf::from("/opt/mychrome"));
}
