...
6. **The shortcuts inhibitor is measured against sway and a virtual keyboard,
   ...
   **Its first run on `crux` read neither end.** The engine segfaulted just
   after the page's listener attached, before any key reached it, and crashpad
   could not read the process, so there is no stack yet. The guard now reports
   the crash as its verdict and prints what the engine wrote and where its core
   went; the crash itself is open.
