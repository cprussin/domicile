...
| A browser window can say a page is still arriving | `guard-webview-history.sh`, in the same schedule: the element reads `false` on a page that landed a step ago and `true` on a navigation the fixture is holding open, with an event behind the change, and reads `false` again the moment `stop()` cancels it. It is the browser's `should_show_loading_ui` and `IsLoading()` resolved into one answer — "a browser would be showing a spinner" — so a same-document navigation does not set it; a `loading` property plus a payload-free `domicile-loading-change` event, for the reason the history pair are properties, and a sharper one: a load is a span, so a chrome that mounts in the middle of one is the ordinary case rather than a race. What it does NOT say is what is arriving — see the address bar gap below |
| A link with `target="_blank"` opens a browser window | `guard-webview-new-window.sh`. A guest cannot be handed a window content made — it has no `SiteInstance` of its own, which is what keeps the user logged in, and `WebContentsImpl::CreateNewWindow` CHECKs that pair — so `WebViewGuest::CreateCustomWebContents` still refuses the window and now reports the address on `WebViewGuestClient::NewWindowRequested`. The element dispatches `domicile-new-window` carrying it, the only one of its four events with a payload: an address nothing is showing yet is not state the element holds, and a property would be a lie between asks. The shell opens the window, because where a window goes is the shell's answer. The guard clicks the link in a real engine and reads the page that then loads in the SECOND `<webview>` — an event is not a window — and its control is an ordinary link in the same guest, which must ask for nothing. What refusing costs: the opener, `window.open`'s handle, the target's name and a POST body do not survive, so what the user gets is a navigation to that address |
...
- **Every display has a window, and each one is told the display it covers.**
  ...
  opens one per display, on the platform that scans out and nowhere else,
  and reconciles that list on every hotplug
  (`ShellWindowsFor` in `//components/domicile:shell_windows`, carried out in
  `//chrome/browser/domicile`). Each window's control channel then states
  which display it is, read by the browser that placed it rather than guessed
  by the page, and the compositor answers that connection with the desktop
  narrowed to that one display at the origin (`Host::as_one_screen`). A shell
  does nothing: `<Screen name="left">` renders in the window on the left
  monitor and nowhere else, which is what it always meant.
...
- **A guest refuses everything an embedder is asked for, and one of those
  refusals is now reported.** Permissions and dialogs route through
  `WebViewGuest`'s `WebContentsDelegate` and are answered by the default. A
  refusal is the answer that has a guard behind it; answering them is one piece
  of work each. The window a page asks for is the one that has been done: it is
  still refused — see the CHECK above — and the address now reaches the shell,
  which opens a browser window of its own. What that window is not is the window
  the page asked for: `window.open` gets `null` back, the opener relationship
  and the target's name are not carried, and a form POSTed at a new target
