#!/usr/bin/env bash
# That neither shortcuts-inhibitor guard returns while anything it started is
# still running.
#
# Each guard starts two things in the background, and on `crux` the pid a `&`
...
# things.
#
# So both are started here through each guard's own lines, out of the real
...
#
# BOTH GUARDS, ONE CHECK. `guard-shortcuts-inhibitor.sh` reads the request off
# the wire and `guard-shortcuts-inhibitor-chord.sh` presses a chord through the
# same kind of keyboard into the same kind of engine; they start the same two
# things the same way, and the invariant is the same one. A second copy of this
# file for the second guard would be two things to keep in step.
set -u
...
# One guard. Run in a subshell of its own, so that its stand-ins, its trap and
# its exit are its own.
leaves_nothing() { # guard script name, the first line of what its cleanup reads, binds, crashes
  ...
  : >"$PIDS"
  # And what the host was asked, by a guard that installs a binding.
...
wait
CHROME
  # OR ONE THAT CRASHED, leaving its children to whoever adopts them: on crux the
  # browser segfaulted and its network process went on for as long as the guard
  # did. Nothing that walks the browser's tree finds them once it is gone.
  if [ "${4:-}" = "crashes" ]; then
    sed -i 's/^wait$/ulimit -c 0; kill -SEGV $$/' "$CHROMIUM/$OUT/chrome"
  fi
  chmod +x "$CHROMIUM/$OUT/chrome"

  # The guard, from its libraries to its exit, in a subshell of its own: the
  # trap it sets fires where the guard's fires, on the way out.
  (
    ...
    eval "$LIBRARIES"
    eval "$CLEANUP"
    # A binding in the host, as the chord guard has once swaymsg took it — the
    # other thing it leaves in a session that outlives it if cleanup does not
    # take it back out. The request guard installs none and reads neither.
...
# A piece of `$GUARD`, between two lines that are whole at column zero. A
# rewrite that moves one fails here rather than leaving this passing against a
