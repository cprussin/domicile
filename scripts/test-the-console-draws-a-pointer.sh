...
for workflow in engine.yml engine-drm-probe.yml; do
  if grep -q "DrmCursorFactoryTest:" "$ROOT/.github/workflows/$workflow" 2>/dev/null; then
    ok "$workflow carries a floor for the suite"
  else
    fail "$workflow carries a floor for the suite" \
      "no 'DrmCursorFactoryTest:<n>' in .github/workflows/$workflow"
  fi
done

if grep -q 'DrmCursorFactoryTest\.\*' "$ROOT/.github/workflows/engine.yml" 2>/dev/null; then
  ok "engine.yml's filtered run names the suite"
else
  fail "engine.yml's filtered run names the suite" \
    "DrmCursorFactoryTest.* is not in the --gtest_filter, so the floor counts a suite that never ran"
fi
