...
| `shadows.lifted`, `shadows.modal` | Domicile preset. `lifted` is a soft, broad drop shadow for floating surfaces (its one consumer here is `Select`'s popup); `modal` is the heavier elevation used by dialogs. |
