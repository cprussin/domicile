...
| [/docs/architecture/BROWSER-WINDOW-PARITY.md](/docs/architecture/BROWSER-WINDOW-PARITY.md) | Why a `<webview>` renders nothing on a site that refuses framing and why no chord reaches the shell over one — a subframe is not a top-level page — and the guest that fixes both. Nothing shipped. |
