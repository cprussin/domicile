...
| `scripts/guard-webview-routed-link.sh`, `guard-webview-routed-link.js`, `guard-webview-routed-link-server.py` | a link the page's own renderer cannot follow — `target="_top"` inside a frame from another site — and the window it has to move. Blink hands such a navigation to the browser, where content's default delegate does nothing at all; the guard reads the engine's line for the routing *and* the page arriving as the top document, so a frame that turned out to be same-process fails rather than passes. `a.test` and `b.test` under `--host-resolver-rules` are what make the frame cross-site; two ports of one host would be one site and one process. Its control is an ordinary link in the same frame, which must move nothing |
