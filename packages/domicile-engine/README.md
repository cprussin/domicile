...
| `upstream/` | bugs found in Chromium itself, written to be filed and not yet filed. Two today: `setoverridechildpaintflags.md`, which `HTMLAppElement`'s own `SurfaceLayerBridge` runs into — see `ENGINE-FORK.md`'s *Whether an `<app>` is an out-of-process `<iframe>`* — and `browser-plugin-embedder-null-guest-manager.md`, the browser-process crash a plain Escape reached through a `<webview>`, which patch 0037 closes here |
...
| `scripts/guard-shortcuts-inhibitor.sh`, `guard-shortcuts-inhibitor.js` | the request patch `0038` makes, read off the engine's own `WAYLAND_DEBUG=1` capture: `inhibit_shortcuts` to the host compositor, which is what keeps a nested desktop's Meta chords from being taken by the session it runs inside. **The request having been made, and nothing further** — no key is pressed and nothing says the host honored it. Its control is the same run with `--domicile-inhibit-host-shortcuts` left off, where the request must be absent, and it puts a virtual keyboard on the host's seat first because the engine asks for nothing against a seat that announced none |
| `scripts/guard-shortcuts-inhibitor-chord.sh`, `guard-shortcuts-inhibitor-chord.js` | whether the host **honored** that request: `Mod4+y` pressed through a virtual keyboard with the desktop's window focused, read on both sides — a sway binding installed over IPC that writes a line when it fires, and the page's own keydown listener. With `--domicile-inhibit-host-shortcuts` the page must get it and sway must not; its control, with the switch off, the reverse. Each side is shown able to see first — the binding fires once at an empty host, a plain key reaches the page — so a key that went nowhere fails rather than passing as "the host did not take it". Needs a sway it can reach, which is why it is not part of the request guard; says nothing about other hosts or a physical keyboard |
...
| `scripts/guard-webview-routed-link.sh`, `guard-webview-routed-link.js`, `guard-webview-routed-link-server.py` | a **middle click** on an ordinary link in a browser window, and the second window it asks for. A page cannot open one, so the browser must, and content's default delegate does nothing at all. The guard reads the engine's own `OpenURLFromTab` line *and* the element's `domicile-new-window` event, because `ReportNewWindow` is shared with the `target="_blank"` path and the event alone cannot say which half asked. Its control is the **same point on the same link** pressed with the left button, which must follow it in place and ask for nothing |
...
A desktop runs under `--app`, and patch 0047 leaves every key the shell does
not handle unhandled, so there is no reload in it to press. `scripts/test-dev-shell.sh`
