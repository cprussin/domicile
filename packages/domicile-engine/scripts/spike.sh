#!/usr/bin/env bash
# Run step 2 of the spike: start the engine with the broker socket open, then
# run the external submitter against it and check what viz drew.
#
...
#
#   ... spike.sh /build/chromium/src --domicile-spike-skip-hierarchy
#   ... spike.sh /build/chromium/src -- --color=FF00C853
#
# Flags before `--` go to the engine, after it to the submitter. Exits 0 only
# if the pixel viz drew is the one the submitter sent.
#
...
#                               no GPU resources, which is half of why step 2
#                               submits them
...
CHROMIUM="${1:-}"
if [ -z "$CHROMIUM" ]; then
  echo "usage: spike.sh <path to chromium/src> [engine flags] [-- submitter flags]" >&2
  exit 1
fi
shift
...
"$OUT/chrome" \
  ...
  "${ENGINE_FLAGS[@]}" \
  about:blank > /tmp/domicile-spike-engine.log 2>&1 &
ENGINE=$!
...
if [ ! -S "$SOCKET" ]; then
  echo "the engine never opened $SOCKET; see /tmp/domicile-spike-engine.log" >&2
  kill $ENGINE 2>/dev/null
  exit 1
fi

timeout 120 "$OUT/domicile_solid_color_submitter" \
  --domicile-broker-socket="$SOCKET" "${SUBMITTER_FLAGS[@]}"
RESULT=$?
