...
# 8732, the history guard's 8733 or the click guard's 8734: two guards on one
# port is two guards that cannot run in the same job, and CI runs them in one.
PORT="${PORT:-8735}"
DEBUG_PORT="${DEBUG_PORT:-9234}"
...
python3 "$SCRIPTS/guard-webview-new-window-server.py" --port "$PORT" \
  >"$HTTP_LOG" 2>&1 &
STARTED+=($!)
wait_for_line 240 "serving" "$HTTP_LOG" || {
  annotate_from "guard-webview-new-window: nothing came up on port $PORT" "$HTTP_LOG"
