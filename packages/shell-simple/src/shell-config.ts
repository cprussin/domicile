...
//
// The shell owns this, not Domicile. Someone running the simple shell runs
// `simple`, and configures it here — there is no compositor to know about and
...
const RELATIVE = path.join("domicile", "simple.json");

/** The environment as a process has it, which is all-optional by nature. */
type Environment = Readonly<Record<string, string | undefined>>;

/** Everything the simple shell can be told. */
...
export const configPath = ({ HOME, XDG_CONFIG_HOME }: Environment): string => {
  if (XDG_CONFIG_HOME !== undefined) {
    ...
    throw new Error(
      "simple: neither $XDG_CONFIG_HOME nor $HOME is set, so there is nowhere to read a config from",
    );
  } else {
