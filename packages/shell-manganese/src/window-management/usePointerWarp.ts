...
/**
 * Take the pointer with the keyboard, so that the focus stays where a key put
 * it — `mouse_warping container` from the config.
 *
 ...
 * to do: which press was a keyed one, where the pointer is, and what to ask
 * the engine for once the window has been laid out at its new box.
 *
 ...
 */
export const usePointerWarp = ({ domicile, focus }: Options): (() => void) => {
  // Refs rather than state, all three: none of them is drawn, and a pointer
